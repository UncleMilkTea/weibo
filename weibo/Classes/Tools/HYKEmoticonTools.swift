...
class HYKEmoticonTools: NSObject {
    ...
    /// 最近表情
    lazy var recentEmoticons:[Emoticons] = {
        
        // 从文件里面读取最近表情
        if let result = NSKeyedUnarchiver.unarchiveObjectWithFile(self.recentEmoPath) as? [Emoticons]{
    ...
    ///返回某种表情二维数组
    private func typeEmoticonPage(emoticons:[Emoticons])->[[Emoticons]]{
        
        //定义一个零时数组
        var array:[[Emoticons]] = [[Emoticons]]()
        //计算共有几页
    ...
    // 读取表情的方法
    private func emoticonsWithFolderName(folderName:String)->[Emoticons]{
        ...
        for value in tempArray {
            ...
            let model = Emoticons(dic: (value as! [String:AnyObject]))
            //记录表情数据文件夹的名字
    ...
    /// 最近表情按钮的保存
    func saveRencentEmoticons(emoticons:Emoticons){
    ...
    /// 通过表情字符串,去查找其对应的表情模型
    func emoticonsWithChs(chs:String)->Emoticons?{
